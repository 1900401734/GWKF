...
namespace MesDatas.Utility
{
    ...
    public class RequestMes
    {
        ...
        /// <returns></returns>
        public T1 GetResponseSerializeResult1<T1, T2>(string url, HttpClientUtil httpClient, string function, T2 inputParam, string logFile)
        {
        ...
        {
            // 优化：直接从对象生成 JObject，避免先转字符串带来的内存损耗
            JObject json = inputParam == null ? new JObject() : JObject.FromObject(inputParam);
            ...
            // 优化：直接将 JObject 转换为指定的泛型对象
            return jsonResult.ToObject<T1>();
        }
